...
function renderLiveTick(){
  try{
    // Always update live DOM elements (price, PNL, values, Bot tab)
    if(typeof window.updateLiveUI === 'function') {
      window.updateLiveUI();
    }
...
function renderPortfolio(){
  ...
  tb.innerHTML=h.map((hh,i)=>{
    ...
    return `<div class="asset-card" onclick="showInteractiveChart('${sym}')">
      <div class="ac-header">
        <div class="ac-coin">
      ...
      <div class="ac-body">
        <div class="ac-row">
          ...
          <div class="ac-col" style="align-items: flex-end; text-align: right;">
            ...
            <div class="ac-sub b-editable" onclick="event.stopPropagation();editCost('${hh.id}')" title="Sửa giá vốn">Vốn: ${F.usd(hh.cost)} ✏️</div>
          </div>
        </div>
...
// NAV + EVENTS
// ═══════════════════════════════════════════════════════
const TITLES={portfolio:'Tài sản của tôi',alerts:'Cảnh báo',research:'Trợ lý AI & Tín hiệu',binance:'Binance',settings:'Cài đặt'};
...
function renderMyCoinsAlerts(){
  ...
  const html=S.holdings.map(h=>{
    ...
    return`<div class="mca-item" onclick="showInteractiveChart('${sym}')">
      <div class="mca-top">
...
function updateBotUI() {
  ...
  if(tbodyPos) {
    if(botState.positions.length === 0) {
      tbodyPos.innerHTML = '<tr><td colspan="6" class="ld" style="text-align:center;padding:20px;color:var(--t4)">Không có vị thế mở</td></tr>';
    } else {
